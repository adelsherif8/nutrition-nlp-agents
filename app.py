...
def generate_meal_plan(targets: dict, profile: UserProfile, model_variant: str = "M1 — Consistent") -> dict:
    ...
    if db:
        for meal_name, items in meal_json.items():
            verified[meal_name] = db.meal([(i["food"],i["grams"]) for i in items])
        achieved = sum(m["macros"]["calories"] for m in verified.values())
        if achieved > 0:
            scale = cal / achieved
            scaled = {}
            for mn, md in verified.items():
                ...
                for item in md["items"]:
                    ng = round(item["grams"]*scale)
                    if db:
                        rc = db.macros(item["food_name"], ng)
                        if rc: ni.append(rc)
